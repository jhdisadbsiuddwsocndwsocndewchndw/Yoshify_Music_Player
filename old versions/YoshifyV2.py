...
song_number = 1
song_info = {
    1: "Another Life, Another love - ryan Stone",
    2: "songgg - yoshi",
    3: "songggg - yoshi",
    4: "song4 - yoshi"
}
...
def open_player(root):
    ...
    playerFrame = tk.Frame(root, bg="purple")
    playerFrame.grid(row=0, column=0)

    img = Image.open("Yoshify (1).png")
    img = img.resize((250, 250))
    image = ImageTk.PhotoImage(img)

    logo = tk.Label(playerFrame, image=image, bg="purple")
    logo.image = image  # keeps a reference so python doesn't delete the image
    logo.grid(row=0, column=1, pady=3)

    songTitleAndArtist = tk.Label(playerFrame, text=song_info[song_number], font=("Arial", 24, "bold"), bg="purple", fg="white")
    ...
    def skipCurrentSong():
        ...
        currentSong = vlc.MediaPlayer(f"songs/{song_number}.mp3")
        # this line is getting the song from the vlc media
        currentSong.play()
    ...
    startBtn = tk.Button(playerFrame, text="Play/Pause", fg="red", command=startStopMusic)
    startBtn.grid(row=4, column=1, padx=5)

    nextBtn = tk.Button(playerFrame, text="Skip track", fg="red", command=skipCurrentSong)
    nextBtn.grid(row=4, column=2, padx=5)

    prevBtn = tk.Button(playerFrame, text="Previous track", fg="red", command=previousSong)
    prevBtn.grid(row=4, column=0, padx=5)

    volumeLabel = tk.Label(playerFrame, text="Volume", bg="purple", fg="white")
    volumeLabel.grid(row=2, column=1, padx=5)

    volumeSlider = tk.Scale(playerFrame, from_=0, to=100, orient="horizontal", length=200, command=changeVolume, bg="purple", fg="white")
