...
# this stops the table from being wiped/recreated every time the program runs
cursor.execute("""
...
# saves the table creation to the database file
connection.commit()


def addUser(username, password):
    # inserts a new row into the users table with the given username and password
    # the ? symbols are placeholders, which keeps the query safe from SQL injection
    cursor.execute(
...
def checkUser(username, password):
    ...
    # if a matching row was found, the login details are correct
    if user:
        return True
    # if no matching row was found, the login details are wrong
    else:
        return False
